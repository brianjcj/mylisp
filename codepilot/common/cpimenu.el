  ...
  (let (pos ll ll2 ind ll3
    ...
    (dolist (ii ll)
      (when (consp ii)
        (cond ((listp (cdr ii))
               (insert prefix-str)
               (insert-image codepilot-image-bucket "ก่") ;; brian test
               (insert " " (car ii) "  \n")
  ...
  (let (pos (loo t) e str)
    (save-excursion
      (save-match-data
        ...
        (while (and loo (re-search-backward "ก่" nil t))
...
(defun cpimenu-fold-all ()
  ...
  (let (e)
    (save-match-data
      (save-excursion
        (goto-char (point-min))
        (while (re-search-forward "ก่" nil t)
...
(defsubst cpimenu-output (buf-name ilist cedet?)
  (let (pos)
    (with-modify-in-readonly
        (save-excursion
          (erase-buffer)
          (insert "[*Rescan*] Sort: [Name] " )
          (setq pos (point))
          (insert "[Position]")
          (put-text-property pos (point) 'face 'custom-button)
          (insert "\n\n")
          (cpimenu-mode)
...
(defun cpimenu ()
  ...
  (let ((buf-name (buffer-name))
        ...
        (mj-mode major-mode)
        buf
        (cedet? (and (featurep 'semantic) semantic--buffer-cache))
        )
    (setq buf (get-buffer-create cpimenu-buf-name))
    (set-buffer buf)
    (setq buffer-read-only t)
...
(defun cpimenu-which-func (&optional taglist)  ;; =ToSync= with which-func.el
  (let (name pos win buf-name update? ooo)
