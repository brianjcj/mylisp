	...
	(progn
	  ...
	  (if cscope-marker
		...
		(setq cscope-marker nil)))
          ...
          ;; brian
          (codepilot-search-and-hl-text cscope-symbol-local nil 'id)
          )
      (message "No entry found at point.")))
  window)
    ...
    (cond ((get-buffer buf-name)
          ...
          (t
           ...
           (let ((outbuf (get-buffer-create cscope-output-buffer-name))
             ...
             (cplist-add-line-to-idlist "^ก่ CScope Query List  " (concat "  " buf-name "\n"))
             )))))

(define-key codepilot-ro-mode-map "u" 'codepilot-previous-buffer)
...
(define-key codepilot-ro-mode-map "g" 'cplist-list-id-section-bufs)  ;; brian
...
(define-key cscope-list-entry-keymap "g" 'cplist-list-id-section-bufs)
